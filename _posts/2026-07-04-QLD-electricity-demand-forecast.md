...
Note that the standard deviation above is generated by the posterior distribution as the Bayesian method provides both the point estimate and its uncertainty. The plot is chosen from the test set of the demand in April 2025.
...
The plot is chosen from the test set of the demand in May 2025.
...
Note that the standard deviation above is computed via the residual between the predictor and the test set.
