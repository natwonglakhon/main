...
**AEMO price and demand data** is pulled directly from the AEMO website for the QLD1 region, covering June 2020 to June 2026. The post-Covid era is chosen intentionally to avoid the structural demand shifts that happened during lockdowns, which would add noise to any model trying to learn normal demand patterns. The two key variables are `TOTALDEMAND` (MW, the target) and `RRP` (Regional Reference Price, $/MWh).

**Weather data** comes from the Open-Meteo historical archive API at hourly resolution, collected for three locations across Queensland: Brisbane (south), Rockhampton (central), and Townsville (north). Covering three locations reflects the geographic scale of Queensland's grid. The variables are apparent temperature, temperature at 2m, dew point, relative humidity, and rainfall. More than half of Queensland's population lives in the southeast, so the expectation going in was that Brisbane weather would dominate the model. That expectation turned out to be wrong, and I'll come back to why.
...
The train/test split uses `shuffle=False` throughout this project to preserve chronological order. Testing on future data the model has never seen gives a much more honest picture of how a real forecasting system would perform.
...
For the tree models, I include the discrete calendar features (day of week, weekend flag, etc.) by one-hot encoding them, giving a richer feature set.
...
Using `shift(freq="7D")` and `shift(freq="366D")` rather than counting steps ensures that the lags align to the correct timestamps even when there are gaps or leap years in the data.
...
- **Hour of day**: critical for encoding the daily demand cycle
...
Feature engineering delivers a significant drop for the tree models, from around 7.5% down to 4.59% for XGBoost. The linear models see only a modest improvement because they can't exploit the nonlinear structure in the engineered features the way tree models can.
